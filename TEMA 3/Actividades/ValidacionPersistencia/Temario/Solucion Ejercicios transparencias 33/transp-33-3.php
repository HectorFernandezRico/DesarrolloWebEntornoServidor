...
<?php
    if (!isset($_GET["texto"])) {
        ...
        if (empty(trim($_GET["texto"]))) {
            $mensaje = "El texto enviado está en blanco";
    ...
    // Persistencia: Si el usuario envió un texto, lo mostraré. Sino, no pondré nada.
    $textoAnterior = isset($_GET['texto']) ? $_GET['texto'] : "";
?>
...
<body>
    ...
    <form class="celeste">
        ...
        <!-- Añadimos htmlentities para desinfectar el texto escrito por el usuario -->
        <input class="separado" id="texto" name="texto" value="<?=htmlentities($textoAnterior);?>"/>
